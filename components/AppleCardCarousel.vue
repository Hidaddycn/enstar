<template>
    <div class="relative w-full">
        <div ref="carouselRef"
            ...
            <div :class="cn('absolute right-0 z-[1000] h-auto w-[5%] overflow-hidden bg-gradient-to-l')"></div>

            <div :class="cn('flex flex-row justify-start gap-4 pl-4', 'mx-auto max-w-7xl')">
                <slot></slot>
            </div>
        </div>
        <div class="mr-10 flex justify-end gap-2 mb-4">
             <button
                class="relative  flex size-10 items-center justify-center rounded-full hover:cursor-pointer !bg-gray-100 disabled:opacity-50 w-10 h-10"
                :disabled="!canScrollLeft" @click="scrollLeft">
                <span class=" text-gray-500"> ←</span>
            </button>
            <button
                class="relative  flex size-10 items-center justify-center rounded-full !bg-gray-100 disabled:opacity-50 w-10 h-10"
...
import { CarouselKey } from "../lib/AppleCarouselContext.ts";
